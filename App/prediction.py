...
def create_lstm_dataset(series, look_back=1):
    """Convierte la serie de tiempo en pares de entrada (X) y salida (Y) para el LSTM."""
    # Requiere que la serie esté escalada
    X, Y = [], []
...
def predict_next_day_anomaly(series, cut_off_date, look_back):
    """
    Entrena el modelo LSTM con datos hasta la fecha de corte y predice el día siguiente.
    """
    
    # Asegurar que la serie sea un objeto datetime para la indexación
    series.index = pd.to_datetime(series.index)
    
    # 1. AISLAR LOS DATOS DE ENTRENAMIENTO (hasta la fecha de corte)
    train_data = series[series.index.date <= cut_off_date].copy()
    
    if len(train_data) < look_back + 1:
        return f"Datos insuficientes para el lookback ({look_back} días). Necesitas más datos históricos.", None
    
    # 2. NORMALIZACIÓN
    scaler = MinMaxScaler(feature_range=(0, 1))
    scaled_train = scaler.fit_transform(train_data.values.reshape(-1, 1))
    
    # 3. PREPARACIÓN DEL DATASET PARA EL ENTRENAMIENTO
    ...
    # 5. CÁLCULO DEL UMBRAL DE ERROR HISTÓRICO (para detección de anomalías)
    train_predict = scaler.inverse_transform(model.predict(X_train)).flatten()
    ...
    # 6. PREPARAR LA ENTRADA PARA LA PREDICCIÓN (últimos 'look_back' días)
    last_window = scaled_train[-look_back:] # Ventana de datos para predecir el día siguiente
    X_predict = np.reshape(last_window, (1, look_back, 1))
    ...
    # 8. EVALUACIÓN DE ANOMALÍA (Si el dato del día siguiente existe en el DF)
    next_day = cut_off_date + pd.Timedelta(days=1)
    ...
    try:
        ...
        actual_next_day_sales = series.loc[series.index.date == next_day.date()].iloc[0]
